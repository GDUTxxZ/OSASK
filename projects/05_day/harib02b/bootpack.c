...
void HariMain(void)
{
	char *vram;
	int xsize, ysize;
	struct BOOTINFO *binfo; // 野指针，值类型是 任何一个 struct BOOTINFO 的地址

	init_palette();
	binfo = (struct BOOTINFO *) 0x0ff0; // 把asmhead.nas里面写入的图像模式数据的基本地址写入binfo指针的值
	xsize = (*binfo).scrnx; // (*binfo)会找到 0x0ff0, .scrnx 会根据数据类型和数据位置进行偏移找到对应的地址，取值
	ysize = (*binfo).scrny;
	vram = (*binfo).vram;
